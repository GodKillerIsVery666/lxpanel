...
export function AuditPage(): JSX.Element {
  ...
  async function pruneAudit(): Promise<void> {
    ...
    try {
      await api.pruneAudit(Number.parseInt(retainDays, 10));
      await load();
    } catch (caught) {
  ...
  return (
    <main className="page-stack">
      ...
      <section className="table-panel">
        ...
        <div className="inline-form wrap"><button type="button" onClick={() => void exportAudit("csv")}><Download size={16} /> CSV</button><button type="button" onClick={() => void exportAudit("jsonl")}><Download size={16} /> JSONL</button><input value={retainDays} onChange={(event) => setRetainDays(event.target.value)} inputMode="numeric" placeholder="保留天数" /><button type="button" onClick={() => void pruneAudit()}><Trash2 size={16} /> 清理</button></div>
      </section>
      <section className="table-panel">
        <table>
