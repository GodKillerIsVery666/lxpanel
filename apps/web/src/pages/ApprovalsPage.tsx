...
export function ApprovalsPage(): JSX.Element {
  ...
  async function create(): Promise<void> {
    try {
      await api.createApproval({ action, target, reason, expiresInMinutes: Number.parseInt(expiresInMinutes, 10), requiredApprovals: Number.parseInt(requiredApprovals, 10) });
      setTarget("");
      setReason("");
      await load();
    } catch (caught) {
  ...
  return (
    <main className="page-stack">
      ...
      <section className="table-panel">
        ...
        <table><thead><tr><th>ID</th><th>动作</th><th>目标</th><th>状态</th><th>进度</th><th>申请人</th><th>到期</th><th>处理</th><th>操作</th></tr></thead><tbody>{approvals.map((approval) => <tr key={approval.id}><td><code>{approval.id}</code></td><td>{approval.action}</td><td>{approval.target}<div className="muted-text">{approval.reason}</div></td><td><StatusPill status={approval.status} /></td><td>{approval.approvedCount}/{approval.requiredApprovals}</td><td>{approval.requestedBy}<div className="muted-text">{formatDate(approval.requestedAt)}</div></td><td>{formatDate(approval.expiresAt)}</td><td>{approval.reviews.length ? approval.reviews.map((review) => <div className="muted-text" key={`${approval.id}-${review.reviewedAt}`}>{review.reviewedBy}: {review.decision}</div>) : approval.reviewedBy ?? "-"}{approval.consumedBy ? <div className="muted-text">used by {approval.consumedBy}</div> : null}</td><td className="row-actions">{approval.status === "pending" ? <><button title="批准" onClick={() => void decide(approval.id, true)}><Check size={15} /></button><button title="驳回" onClick={() => void decide(approval.id, false)}><X size={15} /></button></> : null}</td></tr>)}</tbody></table>
      </section>
    </main>
  );
}
