...
import type { Connector, Host } from "@lxpanel/shared";
...
export function HostsPage(): JSX.Element {
  ...
  async function load(): Promise<void> {
    try {
      const [hostResponse, connectorResponse] = await Promise.allSettled([api.hosts(), api.connectors()]);
        ...
        <table>
          ...
          <tbody>{hosts.map((host) => (
            <tr key={host.id}>
              ...
              <td>{host.id.startsWith("connector:") ? null : <button className="mini-button" onClick={() => void remove(host.id)}><Trash2 size={14} /> 删除</button>}</td>
            </tr>
          ))}</tbody>
        </table>
      </section>
