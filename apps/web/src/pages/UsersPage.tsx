    ...
    <main className="page-stack">
      ...
      <section className="table-panel">
        <table>
          ...
          <tbody>{users.map((user) => <tr key={user.id}><td>{user.username}</td><td><select value={user.role} onChange={(event) => void updateRole(user.id, event.target.value as Role)}><option value="owner">owner</option><option value="operator">operator</option><option value="viewer">viewer</option></select></td><td>{formatDate(user.createdAt)}</td><td>{formatDate(user.lastLoginAt)}</td><td><span style={{ display: "inline-flex", gap: 8, alignItems: "stretch", whiteSpace: "nowrap" }}><input value={resetPassword} onChange={(event) => setResetPassword(event.target.value)} type="password" placeholder="新密码" style={{ width: "auto", flex: "1 1 80px", minWidth: 0 }} /><button className="mini-button" onClick={() => void reset(user.id)}>重置</button></span></td><td><button className="icon-button" onClick={() => void remove(user.id)} title="删除"><Trash2 size={16} /></button></td></tr>)}</tbody>
        </table>
      </section>
    </main>
  );
