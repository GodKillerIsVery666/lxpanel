...
export const api = {
  ...
  syncPluginManifest: (input: { url: string; trustMode: string; publicKey?: string }) => request("/api/platform/plugins/sync-remote", z.object({ plugin: PluginManifestSchema }), "POST", input),
  ...
  generateLicense: (input: { plan: string; licensedTo: string; machineCode?: string; expiresAt?: string }) => request("/api/platform/license/generate", z.object({ result: LicenseGenerationResultSchema }), "POST", GenerateLicenseSchema.parse(input)),
