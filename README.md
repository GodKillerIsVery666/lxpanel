...
- 应用商店基础：内置 Nginx、Redis、PostgreSQL Docker Compose 模板，展示模板来源、签名和验证状态，支持渲染部署记录、启动/停止/重启、变量升级、健康检查和上一版本回滚。
...
- 数据库管理：登记 PostgreSQL、MySQL、MariaDB 连接，服务端加密保存连接 URL，前端只显示脱敏地址，并提供手动备份、备份保留天数和恢复演练入口。
...
- 审计日志：支持筛选、CSV/JSONL 导出、哈希链完整性校验、合规统计和带审批的保留清理；安全态势页结构化检查会话密钥、HTTPS Cookie、IP 白名单、备份、状态存储、Docker socket 与 SSH 配置，并提供防火墙、SSH、Docker socket 加固计划；CI、类型检查、单元测试和 E2E smoke。
- 平台治理：提供资源级访问策略、访问评估、自动化安全修复 dry-run、容量计划、升级向导、离线交付清单和 OpenAPI/Webhook 摘要。
...
平台页面向商业交付场景，集中展示资源级访问策略、审计完整性、合规统计、安全修复 dry-run、容量建议、升级前检查、离线交付清单和开放 API/Webhook 事件摘要。`owner` 与 `operator` 可使用治理写接口，自动化调用需要 `platform:read` 或 `platform:write` 作用域。
