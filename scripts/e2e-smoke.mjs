...
try {
  ...
  const checks = [
    ...
    await app.inject({ method: "GET", url: "/api/platform/tenant-report?workspace=default", headers: { cookie } }),
    await app.inject({ method: "GET", url: "/api/platform/connectors/version-policy", headers: { cookie } }),
    await app.inject({ method: "POST", url: "/api/platform/connectors/upgrade", headers: { "content-type": "application/json", cookie }, payload: JSON.stringify({ channel: "stable", rolloutPercent: 100 }) }),
