...
前端导航配置集中在 `apps/web/src/navigation.ts`，同一份元数据同时驱动侧边栏、角色过滤、页面记忆、最近访问和首页快捷入口。Shell 按总览、运维、文件与自动化、安全与交付分组，并支持按功能、资源和动作搜索。概览页作为日常工作台，聚合资源指标、安全提醒、告警状态和角色感知快捷入口，让用户打开面板后可以直接进入主机、应用、数据库、备份、安全或平台治理流程。
